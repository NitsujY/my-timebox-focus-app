...
- **Fast add**: each section has a ghost "＋ Add…" row; input stays open after Enter for rapid entry. Supports mini syntax: `Fix VPN 15m #Focus p1` (duration, target section, priority — p1 = urgent). Desktop: `N` opens the Focus input, `B` the Buffer input.
